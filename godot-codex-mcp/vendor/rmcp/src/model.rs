...
// `Option<Value>` normally maps both a missing field and an explicit JSON null
// to `None`. Elicitation approval is security-sensitive: callers must be able
// to reject explicit null while accepting an omitted action-only content field.
// `default` handles omission, while this deserializer preserves every present
// JSON value, including null, inside `Some`.
