...
async fn test_elicitation_result_serialization() {
    ...
    // Presence is significant for action-only approval responses. An omitted
    // content field remains None, while explicit null must survive as a value
    // so the server can reject it instead of treating it as omission.
    let omitted: ElicitResult = serde_json::from_value(json!({"action": "accept"})).unwrap();
    assert_eq!(omitted.content, None);
    let explicit_null: ElicitResult =
        serde_json::from_value(json!({"action": "accept", "content": null})).unwrap();
    assert_eq!(explicit_null.content, Some(json!(null)));
    assert_eq!(
        serde_json::to_value(explicit_null).unwrap(),
        json!({"action": "accept", "content": null})
    );
    let empty_object: ElicitResult =
        serde_json::from_value(json!({"action": "accept", "content": {}})).unwrap();
    assert_eq!(empty_object.content, Some(json!({})));
