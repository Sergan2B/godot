...
TEST_CASE("[CodexS10Persistence] Staging is read-only and commit restore preserve exact file state") {
	...
	String code;
	String message;
	REQUIRE(ScopedPersistenceExecutor::stage(plan, Dictionary(), prepared, code, message) == OK);
...
TEST_CASE("[CodexS10Persistence] Stale preimages and external postimage edits block writes and restore") {
	...
	String code;
	String message;
	CHECK(ScopedPersistenceExecutor::stage(stale_plan, Dictionary(), prepared, code, message) == ERR_INVALID_DATA);
