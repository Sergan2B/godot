...
pub struct SegmentStore {
    ...
    project_id: String,
    initial_version: u32,
    active_cache: Option<SegmentCache>,
    _lock: File,
}
...
impl SegmentStore {
    ...
    pub fn open_with_version(
        ...
        match store.active_manifest() {
            Ok((manifest, _)) => {
                ...
                store.active_cache =
                    Some(SegmentCache::new(load_generation(&store.root, &manifest)?));
            }
    ...
    fn activate_with_version(
        ...
        progress.finish()?;
        let resource_lookup = if physical_version >= 2 {
        ...
        self.active_cache = Some(SegmentCache::new(generation.clone()));
...
impl SpikeStore for SegmentStore {
    ...
    fn migrate_v2(&mut self) -> Result<IndexMetadata, StoreError> {
        let (manifest, _) = self.active_manifest()?;
        if manifest.physical_version >= 2 {
        ...
        generation.generation_id = format!("segment-migration-{:020}", generation.index_revision);
...
fn load_generation(root: &Path, manifest: &SegmentManifest) -> Result<IndexGeneration, StoreError> {
    if !(1..=CURRENT_PHYSICAL_VERSION).contains(&manifest.physical_version) {
        return Err(StoreError::IncompatibleSchema);
    }
    ...
    if manifest.physical_version >= 2 {
        let digest = manifest
            .resource_lookup
            .as_ref()
            .ok_or_else(|| StoreError::CorruptStore("v2 resource lookup missing".to_owned()))?;
...
impl StagingProgress {
    fn new(total: usize, fault: Option<FaultInjection>) -> Self {
        Self {
    ...
    fn advance(&mut self, records: usize) -> Result<(), StoreError> {
        ...
        if !self.triggered && self.written >= self.threshold {
            self.triggered = true;
            hit(self.fault, FaultPoint::Staging)?;
        }
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn activation_is_atomic_and_reuses_content_addressed_segments() {
        let temp = TempDir::new().expect("temp");
        let first = synthetic_generation(32, 96, 1);
        let mut store = SegmentStore::open(temp.path(), &first.project_id).expect("store");
        store.activate(&first, None).expect("first activation");
        let before = fs::read_dir(store.root.join("segments"))
            .expect("segments")
            .count();
        let second = renamed_generation(&first, 2);
        store.activate(&second, None).expect("rename activation");
        let after = fs::read_dir(store.root.join("segments"))
            .expect("segments")
            .count();
    ...
    fn v1_to_v2_migration_preserves_logical_graph() {
        ...
        let mut store =
            SegmentStore::open_with_version(temp.path(), &first.project_id, 1).expect("v1");
    ...
    #[test]
    fn storage_neutral_interfaces_drive_segment_candidate() {
        let temp = TempDir::new().expect("temp");
