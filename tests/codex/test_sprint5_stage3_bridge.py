#!/usr/bin/env python3
"""Regression tests for the fail-closed Sprint 5 S5-03 evidence policy."""
...
import sprint5_stage3_bridge as evidence  # noqa: E402


class Sprint5Stage3BridgeTests(unittest.TestCase):
    ...
    def test_source_manifest_is_sorted_self_bound_and_non_circular(self) -> None:
        ...
        self.assertIn("tests/codex/sprint5_stage3_source_scopes.txt", scopes)
        self.assertNotIn("tests/codex/evidence/sprint-5-stage-3-bridge.json", scopes)
    ...
    def test_profile_tampering_fails_closed(self) -> None:
        tampered = copy.deepcopy(self.report)
        tampered["profile"]["manifest_cases"] = 100  # type: ignore[index]
        with self.assertRaisesRegex(evidence.EvidenceError, "profile differs"):
            evidence.validate_evidence(tampered, check_source=False)

    def test_duplicate_evidence_members_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="codex-s5-bridge-evidence-") as directory:
            path = Path(directory) / "duplicate.json"
            path.write_text('{"schema_version":1,"schema_version":1}', encoding="utf-8")
            with self.assertRaisesRegex(evidence.EvidenceError, "duplicate JSON member"):
                evidence.strict_json_load(path)

    def test_evidence_contains_no_source_or_absolute_paths(self) -> None:
