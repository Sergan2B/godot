...
| `PROTOCOL-001` | Bridge RPC 1.0–1.4 locally verified through Sprint 5 `S5-03` | `S5-04` production script adapter and independent review |
...
| `SCRIPT-001` script contract | D-07, contract vectors, independent oracle, and Bridge RPC 1.4 profile locally verified on macOS arm64 | Implement `ScriptSemanticAdapter` and bounded journal |
| Sprint 5 plan | In progress; `S5-01`–`S5-03` complete locally, `S5-04`–`S5-10` remain | Execute `S5-04`: adapter, snapshot/delta journal, invalidation, and gap recovery |
