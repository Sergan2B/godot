...
lease with bounded backoff. While waiting it uses only an authenticated Bridge
health probe and never requests a semantic snapshot. When the owner exits, the
operating system releases the lease and one standby automatically takes
ownership, starts full replication, and leaves the remaining standbys
diagnostic-only. The transaction journal coordinator starts only for that
owner, so index and transaction write authority cannot split between tasks.
Data and indexes are written as new files and flushed before their staging
manifest is renamed into `generations/`.
