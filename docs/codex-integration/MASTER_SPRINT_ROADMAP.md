...
7. `SCRIPT-001-gdscript-and-csharp-adapters.md`
...
Локальная реализация, проверка и source commits Sprint 1 завершены; push, PR и удалённый CI остаются отдельными gate. Sprint 2 также локально завершён: production Rust sidecar, три project-scoped read-only MCP tool и live editor→bridge→sidecar→MCP slice проверены на Windows x86_64 и macOS arm64 по [SPRINT-2-EVIDENCE](SPRINT-2-EVIDENCE.md). Необязательный human-facing Codex UX capture не блокирует автоматический Architecture Proof. Следующий implementation milestone — Sprint 3: ResourceUID и граф файловых зависимостей по исполнимому [SPRINT-3-PLAN](SPRINT-3-PLAN.md). Публикация и remote CI выполняются только по отдельной авторизации.
