...
Локальная реализация и проверка Sprint 1 и Sprint 2 завершены. Sprint 3 также принят: ResourceUID, прямой/обратный граф зависимостей, Bridge RPC 1.2, persistent segment index и два resource MCP tool прошли полный macOS/Windows evidence по [SPRINT-3-EVIDENCE](SPRINT-3-EVIDENCE.md); канонический D-05 выбирает segment store. Следующий implementation milestone — Sprint 4: structural `PackedScene`/`SceneState` index. Публикация и remote CI выполняются только по отдельной авторизации.
