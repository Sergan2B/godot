...
Локальная реализация, проверка и source commits Sprint 1 завершены; push, PR и удалённый CI остаются отдельными неисполненными gate. Следующий implementation milestone — Sprint 2: production Rust sidecar и первый read-only semantic bridge slice. Публикация Sprint 1 и remote CI выполняются только по отдельной авторизации; MCP, semantic index и чтение сцен не переносятся обратно в Sprint 1.
