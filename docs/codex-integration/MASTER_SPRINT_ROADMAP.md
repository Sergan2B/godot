...
`S5-01`–`S5-07` закрыты локально на macOS arm64: канонический
...
cancel/crash/corruption recovery. Единый semantic coordinator независимо
...
`S5-09`. Следующий gate — два symbol MCP tool `S5-08`; полная Sprint 5 приёмка
пока не заявляется.
