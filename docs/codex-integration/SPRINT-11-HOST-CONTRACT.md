...
| Godot × Codex package | workspace `0.1.7` | same-project lease contention, long project-root UDS transport, accepted-socket subprocess inheritance, full-beta doctor registry probing, and current host-measurement guidance fixed; independently installable host-surface bundle added |
...
Package `0.1.7` keeps package, Godot, protocol, schema, registry, Bridge, and
