...
Packages `0.1.0` through `0.1.3`, plus every operator workspace created for
their Sprint 11 acceptance, are superseded by the independently issued
`0.1.4` candidate. Packages `0.1.2` and `0.1.3` remain immutable historical
evidence: `0.1.2` cannot qualify long project roots, while `0.1.3` allowed
accepted Bridge UDS descriptors to survive into a launched game process. Do
not replace old binaries in place, reuse an old detached manifest, or relabel
old evidence. Install `0.1.4` into a clean operator root and repeat App, CLI,
and IDE technical acceptance from the beginning.
...
Package `0.1.4` can consume a later, independently released App/CLI/IDE
...
`0.1.4` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
