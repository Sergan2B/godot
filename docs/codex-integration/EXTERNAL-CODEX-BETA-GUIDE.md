...
Packages `0.1.0` through `0.1.4`, plus every operator workspace created for
their Sprint 11 acceptance, are superseded by the independently issued
`0.1.5` candidate. Packages `0.1.2` through `0.1.4` remain immutable historical
...
`0.1.4` rejected its own full-beta MCP registry during `doctor`. Do not replace
old binaries in place, reuse an old detached manifest, or relabel old evidence.
Install `0.1.5` into a clean operator root and repeat App, CLI, and IDE
technical acceptance from the beginning.
...
Package `0.1.5` can consume a later, independently released App/CLI/IDE
...
`0.1.5` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
