...
`0.1.15` candidate. Packages `0.1.2` through `0.1.14` remain immutable historical
...
produce qualifying surface evidence. Do not replace old binaries in place,
reuse an old detached manifest, or relabel old evidence. Install `0.1.15` into
...
Package `0.1.15` can consume a later, independently released App/CLI/IDE
...
`0.1.15` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
