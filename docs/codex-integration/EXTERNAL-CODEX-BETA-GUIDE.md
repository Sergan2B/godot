...
`0.1.13` candidate. Packages `0.1.2` through `0.1.12` remain immutable historical
...
as `project_binding_mismatch`. Do not replace old binaries in place, reuse an
old detached manifest, or relabel old evidence. Install `0.1.13` into
...
Package `0.1.13` can consume a later, independently released App/CLI/IDE
...
`0.1.13` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
