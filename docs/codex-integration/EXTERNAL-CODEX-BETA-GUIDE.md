...
Packages `0.1.0` through `0.1.2`, plus every operator workspace created for
their Sprint 11 acceptance, are superseded by the independently issued
`0.1.3` candidate. Package `0.1.2` remains immutable historical evidence but
cannot qualify long project roots because its Bridge transport used only a
project-local Unix-domain socket. Do not replace old binaries in place, reuse
an old detached manifest, or relabel old evidence. Install `0.1.3` into a
clean operator root and repeat App, CLI, and IDE technical acceptance from the
beginning.
...
Package `0.1.3` can consume a later, independently released App/CLI/IDE
...
`0.1.3` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
