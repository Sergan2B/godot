...
`0.1.19` candidate. Packages `0.1.2` through `0.1.18` remain immutable historical
...
host measurement. Package `0.1.7` carries an operator fault harness for the
...
embedded absolute foreign project `cwd`. Package `0.1.10` replaced that with a
relative explicit `cwd = "."`, which Codex App resolves from its long-lived
orchestrator instead of the selected task and therefore rejects valid startup
as `project_binding_mismatch`. Package `0.1.13` can strand an incremental editor
snapshot after a client timeout and then deny stop/pause/continue while the
semantic replica recovers. Package `0.1.14` records the official hosts'
...
Package `0.1.17` treated a transient resource/scene checkpoint advance during
script snapshot capture as an editor-session change, disconnected the semantic
coordinator, and could roll back an otherwise valid persisted change set.
Package `0.1.18` could retain a persisted change-set proof baseline while a
...
3. Keep the installer-owned `current` link intact. Project setup writes the
   absolute stable launcher for operations
   `~/Library/Application Support/GodotCodex/current/bin/godot-codex` with the
   package-owned `mcp` prelaunch command and no explicit stdio `cwd`; Codex then
   supplies the current task runtime directory, and App/IDE startup never
   depends on a shell `PATH`. Before starting the sidecar,
...
Package `0.1.19` can consume a later, independently released App/CLI/IDE
...
`0.1.19` package under `S11_CAPTURE_DATA_ROOT`; the fault-phase config or receipt
